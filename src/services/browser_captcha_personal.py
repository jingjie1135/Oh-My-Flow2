...
class _NodriverPatchedSendTransaction(asyncio.Future):
    ...
    def __init__(
        ...
        if params:
            params = params.pop()
        self.message: str = json.dumps({"method": method, "params": params, "id": tx_id})

    def __call__(self, response: Optional[Dict[str, Any]] = None, **kwargs: Any) -> None:
        if self.done():
            return

        payload: Dict[str, Any] = response or kwargs
        if "error" in payload:
            self.set_exception(RuntimeError(str(payload["error"])))
            return

        try:
            self._cdp_obj.send(payload.get("result"))
...
def _patch_nodriver_connection_instance(connection_instance):
    ...
    async def patched_send(self, cdp_obj, _is_update=False):
        ...
        transaction = _NodriverPatchedSendTransaction(cdp_obj, tx_id)
